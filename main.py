...
class Bot(discord.Client):
    ...
    async def on_member_join(self, member):
        ...
        await member.dm_channel.send(f"Hi {member.name}, Bem vindo ao meu servidor!")
    ...
    async def on_message(self, message):
        ...
        if message.content.lower().startswith("$entrar"):
            try:
                ...
                await message.channel.send(
                    "Ein Error: ```{error}```".format(error=error)
                )
        ...
        if message.content.lower().startswith("$sair"):
            try:
                ...
                await message.channel.send(
                    "Ein Error: ```{error}```".format(error=Hugo)
                )
        if message.content.lower().startswith("$play"):
            try:
                ...
                server = message.guild.voice_client
                try:
                    player = await server.create_ytdl_player(
                        "ytsearch: {}".format(yt_url)
                    )
                    players[message.server.id] = player
                    player.start()

                    mscemb2 = discord.Embed(title="Música para tocar:", color=self.COR)

                    mscemb2.add_field(name="Nome:", value="`{}`".format(player.title))
                    mscemb2.add_field(
                        name="Visualizações:", value="`{}`".format(player.views)
                    )
                    mscemb2.add_field(
                        name="Enviado em:", value="`{}`".format(player.upload_date)
                    )
                    mscemb2.add_field(
                        name="Enviado por:", value="`{}`".format(player.uploader)
                    )
                    mscemb2.add_field(
                        name="Duraçao:", value="`{}`".format(player.duration)
                    )
                    mscemb2.add_field(name="Likes:", value="`{}`".format(player.likes))
                    mscemb2.add_field(
                        name="Deslikes:", value="`{}`".format(player.dislikes)
                    )
        ...
        if message.content.lower().startswith("$gitstats"):
            ...
            if len(menssagem) > 2:
                await message.channel.send(
                    "Mais parametros que o esperado. Digite '$help' para receber ajuda."
                )
            elif len(menssagem) == 1:
                await message.channel.send(
                    "Menos parametros que o esperado. Digite '$help' para receber ajuda."
                )
            else:
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if reaction.emoji == "📙" and msg.id == self.msg_id:  # and user == msg_user:
            role = discord.utils.find(
                lambda r: r.name == "Cientista de dados", msg.guild.roles
            )
            await user.add_roles(role)
            print("add")
